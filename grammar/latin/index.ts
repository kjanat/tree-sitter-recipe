 ...
 * @param s - Raw multiword abbreviation string
 * @returns Regex-safe pattern
 */
const toMultiwordPattern = (s: string): string => s.replace(/\./g, "\\.").replace(/\s+/g, "\\s+");

/**
 * Compile a multiword-abbreviation list into a single alternation regex.
 * @param tokens - Multiword abbreviation strings
 * @returns Combined alternation pattern
 */
const buildMultiwordRegex = (tokens: readonly string[]): RegExp => new RegExp(tokens.map(toMultiwordPattern).join("|"));

/** Pre-compiled regex for timing multiword abbreviations. */
const TIMING_MULTIWORD_RE: RegExp = buildMultiwordRegex(TIMING_MULTIWORD);
/** Pre-compiled regex for route multiword abbreviations. */
const ROUTE_MULTIWORD_RE: RegExp = buildMultiwordRegex(ROUTE_MULTIWORD);
/** Pre-compiled regex for dispensing multiword abbreviations. */
const DISPENSING_MULTIWORD_RE: RegExp = buildMultiwordRegex(DISPENSING_MULTIWORD);
/** Pre-compiled regex for forms multiword abbreviations. */
const FORMS_MULTIWORD_RE: RegExp = buildMultiwordRegex(FORMS_MULTIWORD);
/** Pre-compiled regex for conditional multiword abbreviations. */
const CONDITIONAL_MULTIWORD_RE: RegExp = buildMultiwordRegex(CONDITIONAL_MULTIWORD);
/** Pre-compiled regex for compounding multiword abbreviations. */
const COMPOUNDING_MULTIWORD_RE: RegExp = buildMultiwordRegex(COMPOUNDING_MULTIWORD);
